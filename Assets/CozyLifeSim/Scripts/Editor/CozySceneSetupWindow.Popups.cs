...
namespace CozyLifeSim.Editor
{
    public partial class CozySceneSetupWindow
    {
        ...
        private Transform GetPrefabsHolderTransform(ref bool isSceneDirty)
        {
            ...
            for (int i = 0; i < canvas.transform.childCount; i++)
            {
                ...
                if (child.name != "Prefabs_Holder")
                {
                    continue;
                }

                if (holder == null || child.childCount > holder.childCount)
                {
                    holder = child;
                }
            }
            ...
            if (holder == null)
            {
                GameObject holderGo = new GameObject("Prefabs_Holder");
                holderGo.transform.SetParent(canvas.transform, false);
                holderGo.AddComponent<RectTransform>();
            ...
            {
                Transform duplicate = canvas.transform.GetChild(i);
                if (duplicate == holder || duplicate.name != "Prefabs_Holder")
                {
                    continue;
                }

                for (int j = duplicate.childCount - 1; j >= 0; j--)
                {
                    Transform duplicateChild = duplicate.GetChild(j);
                    if (holder.Find(duplicateChild.name) == null)
                    {
                        duplicateChild.SetParent(holder, false);
                        isSceneDirty = true;
                    }
                }
